...
let mix_int i1 i2 t = int_of_float (mix (float_of_int i1) (float_of_int i2) t)
...
let spike_left_img =
  Image.get (Image.Rotation (0.5 *. pi, Image.Ppm_file "img/spike.ppm"))

let spike_down_img =
  Image.get (Image.Rotation (pi, Image.Ppm_file "img/spike.ppm"))

let spike_right_img =
  Image.get (Image.Rotation (1.5 *. pi, Image.Ppm_file "img/spike.ppm"))
...
let draw_fan (f : fan) =
  ...
  for i = 0 to n do
    ...
    let t' =
      mod_float
        (abs_float
          ( (f.position.x +. float_of_int i -. (300. *. f.strength *. time))
          /. 500. ))
        1.
    in
...
(* Builds a poly line from a function f that takes a parameter t in [0; 1] and
  returns a vec *)
...
let create_line a b t = mix_vec a b t
...
let create_rope_line a b l =
  ...
  let f z =
    (sqrt ((l ** 2.) -. ((y2 -. y1) ** 2.)) /. (x2 -. x1)) -. (sinh z /. z)
  in
  let z = find_zero f 1. in
  let a = (x2 -. x1) /. 2. /. z in
  let p =
    (x1 +. x2 -. (a *. log ((l +. y2 -. y1) /. (l -. y2 +. y1)))) /. 2.
  in
  let q = (y2 +. y1 -. (l *. cosh z /. sinh z)) /. 2. in
  let y x = (a *. cosh ((x -. p) /. a)) +. q in
  fun t ->
    let x = mix x1 x2 t in
    { x; y = y x }
...
(* Opens a window, calls step periodically, calls handle_event when an event is
  emitted *)
let run step handle_event size g =
  ...
  let _ =
    Unix.setitimer Unix.ITIMER_REAL
      { it_interval = tick_rate; it_value = tick_rate }
  in
  ();
  try
    let events =
      [ Graphics.Button_down; Graphics.Mouse_motion; Graphics.Key_pressed ]
    in
