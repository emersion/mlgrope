...
let box_point a b pos = is_between a.x b.x pos.x && is_between a.y b.y pos.y
...
let lines a b a' b' =
  (* k*x + y0 = k'*x + y0' x*(k - k') = y0' - y0 x = (y0' - y0) / (k - k') *)
  if a =: a' && b =: b' then
    Some a
  else if a.x = b.x then
    line_vertical a' b' a.x
  else if a'.x = b'.x then
    line_vertical a b a'.x
  else
    ...
    let x = (y0' -. y0) /. (k -. k') in
    let y, y' = (f x, f' x) in
    if abs_float (y -. y') <= 1. then
      Some { x; y }
    else
      None

let segment_point a b pos = box_point a b pos && line_point a b pos

let segments a b a' b' =
  match lines a b a' b' with
  | Some pt ->
    if box_point a b pt && box_point a' b' pt then
      Some pt
    else
      None
  | None -> None
...
let circle_line center radius a b =
  ...
  let ac = center -: a in
  let d =
    Math2d.length { x = abs_float ((u.x *. ac.y) -. (u.y *. ac.x)); y = 0. }
    /. Math2d.length u
  in
  if d <= radius then
    Some a
...
let circle_seg_norm a b center =
  ...
  let ac = center -: a in
  let n =
    { x = -.u.y *. ((u.x *. ac.y) -. (u.y *. ac.x))
    ; y = u.x *. ((u.x *. ac.y) -. (u.y *. ac.x))
    }
  in
  normalize n
...
let polygon_point l pos =
  ...
  let n =
    fold_segments
      (fun n (a', b') ->
        if is_some (segments a b a' b') then
          n + 1
        else
          n)
      0 l
...
let polygon_line _ _ _ = None

(* TODO *)

let polygon_segment _ _ _ = None
...
let polygon_circle l center radius =
  fold_segments
    (fun acc (a, b) ->
      if is_some (circle_segment center radius a b) then
        (a, b) :: acc
      else
        acc)
    [] l
