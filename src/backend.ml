...
let spike_vertices (s : spike) =
  ...
  ; v
    +: { x = radius *. cos (agl +. (2. *. Util.pi /. 3.))
      ; y = radius *. sin (agl +. (2. *. Util.pi /. 3.))
       }
  ; v
    +: { x = radius *. cos (agl +. (4. *. Util.pi /. 3.))
      ; y = radius *. sin (agl +. (4. *. Util.pi /. 3.))
       }
  ]
...
let check_collision pos ent =
  ...
  match ent with
  | Ball b ->
    let d = squared_distance b.position pos in
    Mlgrope.ball_radius *. 2. *. (Mlgrope.ball_radius *. 2.) >= d
  | Goal g ->
    let d = squared_distance g.position pos in
    if sqdRadius >= d then
      raise (TouchedGoalException pos)
    else
      false
  | Star s ->
    let d = squared_distance s.position pos in
    sqdRadius >= d
  | Bubble bu ->
    let d = squared_distance bu.position pos in
    (Mlgrope.ball_radius +. bu.radius) *. (Mlgrope.ball_radius +. bu.radius)
    >= d
  (* | Magnet(m) -> squared_distance m.position pos <= m.radius *. m.radius *)
  | Rope r -> squared_distance r.position pos >= r.length *. r.length
  | Block b ->
    List.length (Collide.polygon_circle b.vertices pos Mlgrope.ball_radius)
    >= 1
  (* | Fan(f) -> Kills you if you get in it or not ? *)
  | Spike s ->
    let vertices = spike_vertices s in
    if
      List.length (Collide.polygon_circle vertices pos Mlgrope.ball_radius)
      >= 1
    then
      raise DestroyBallException
    else
      false
  | _ -> false
    ...
    (fun acc e ->
      if not (List.mem e acc) then
        ...
        | Fan f ->
          ...
          let b = a +: f.size in
          if Collide.box_point a b p then
            e :: acc
          else
            acc
        | Magnet m ->
          if squared_distance m.position pos <= m.radius *. m.radius then
            e :: acc
          else
            acc
        | _ -> acc
...
let compute_forces pos linkList =
  let forceList, isBubble =
    List.fold_left
      (fun acc e ->
        match e with
        | Bubble _ ->
          ({ gravity with y = 0.5 *. abs_float gravity.y } :: fst acc, true)
        | Magnet m ->
          let d = squared_distance m.position pos in
          let d = max 10. (abs_float d) in
          ((magnet_force d m *: direction pos m.position) :: fst acc, snd acc)
        | Elastic e ->
          let sqd = squared_distance pos e.position in
          if sqd >= e.length *. e.length then
            ( (elastic_force sqd e *: direction pos e.position) :: fst acc
            , snd acc )
          else
            acc
        | Fan f ->
          ( (200. *. f.strength *: { x = cos f.angle; y = sin f.angle })
            :: fst acc
          , snd acc )
        | _ -> acc)
      ([], false) linkList
...
let compute_reaction pos sumForces colList linkList =
  List.fold_left
    (fun acc e ->
      ...
      | Rope r ->
        if List.mem e linkList && distance pos r.position >= r.length then
          projection (-1. *: sumForces) (r.position -: pos) +: acc
        else
          acc
      | Block b ->
        let l = Collide.polygon_circle b.vertices pos Mlgrope.ball_radius in
        List.fold_left
          (fun acc (a, b) -> projection (-1. *: sumForces) (b -: a) +: acc)
          sumForces l
      | _ -> acc)
    sumForces colList
...
let apply_constraint ball sumForces col linkList =
  ...
  | Rope r ->
    ...
    then
      projection ball.speed sumForces
    else
      ball.speed
  | Block bl ->
    let l =
      Collide.polygon_circle bl.vertices ball.position Mlgrope.ball_radius
    in
    List.fold_left
      (fun _ (a, b) ->
...
let rec apply_constraints b sumForces colList linkList =
  match colList with
  | h :: t ->
    let b = { b with speed = apply_constraint b sumForces h linkList } in
...
let compute_position pos colList linkList =
  List.fold_left
    (fun _ e ->
      ...
      | Rope r ->
        if List.mem e linkList && distance pos r.position >= r.length then
          (r.length *: direction r.position pos) +: r.position
        else
          pos
      | Block bl -> (
        let l = Collide.polygon_circle bl.vertices pos Mlgrope.ball_radius in
        try
...
let sum_vec l = List.fold_left ( +: ) vec0 l
...
let ball_move ball gs dt gs' =
  ...
  let speed =
    apply_constraints ball sumForces colList links +: (dt *: sumForces)
  in
  let position =
    compute_position (ball.position +: (dt *: speed)) colList links
  in
